...
function Description() {
    ...
    const getQuestion = async () => {
        const res = await axios.get(`http://localhost:3005/api/v1/problems/${router.id}`);
    ...
    useEffect(() => {
        if(router && router.id)
        setProblem(problemsList[router?.id]);
    }, [router])
    ...
    useEffect(() => {
        if(problem && problem.codeStubs && problem.codeStubs.length > 0){
            ...
            setCode(problem.codeStubs[0].userSnippet)
        }
    }, [problem])
        ...
        >
            ...
            <div className='leftPanel h-full overflow-auto' style={{ width: `${leftWidth}%`}}>
                ...
                <div className='markdownViewer p-[20px] basis-1/2'>
                    <h2 className='mb-4'>{problem?.title}</h2>
